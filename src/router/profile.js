                        ...
                        if (err) {
                          res.status(500).send({
                             ...
                             if (results[0].student_profile_picture !== null)
                              {                              
                              await imageToBase64(results[0].student_profile_picture) 
                                  .then(
                                      (response) => {
                          ...
                          if (result[0].school_cover_image !== null )
                          {
                            ...
                            res.status(200).send({
                              status: true,
                              profile: {...results[0], ...result[0],...emailAndUserType }
                            });
                        }
                      });
                  }
      ...
      (err, result) => {
        if (err) {
          ...
          var studentDetails = {
            ...
            dob: req.body.dob,
            student_address:req.body.addressLineOne+', '+req.body.addressLineTwo+', '+req.body.city+', '+req.body.state+'-'+req.body.postalCode,
...
router.post("/updateClassofStudent", auth, async (req, res) => {
    try {
        await connection.query(
            "update student_profile set ? where student_id = ?", [{ class_id : req.body.classId,last_updated_on: new Date() }, req.body.studentId],
            async (err, results, fields) => {
                if (err) {
                    res.status(500).send
                        ({
...
router.delete("/removeStudent/:id", auth, async (req, res) => {
    try {
        ...
        connection.query("DELETE FROM student_with_school WHERE student_id = ?", _id,
            async (err, results, fields) => {
                if (err) {
                    res.status(500).send({
                        status: false,
                    ...
                    connection.query("DELETE FROM student_profile WHERE student_id = ?", _id,
                        async (err, results, fields) => {
                            if (err) {
                                res.status(500).send({
                                    status: false,
                                 ...
                                 connection.query("DELETE FROM credentials WHERE user_id = ?", _id,
                                  async (err, results, fields) => {
                                      if (err) {
                                          res.status(500).send({
                                              status: false,
...
router.get("/studentFromSchool", auth, async (req, res) => {
    try {
        await connection.query("SELECT * FROM student_profile WHERE student_id in (SELECT student_id From student_with_school WHERE user_id = ?)",
            req.user_id,
            (err, results, fields) => {
                if (err) {
                    res.status(500).send({
...
router.post("/saveImage", auth,async (req, res) => {
  ...
  try {
    ...
    let imageUrl;
    await bucket.file(imageInfo.fileName).getSignedUrl({
                                action: 'read',
                                expires: '03-09-2491'
                              }).then(signedUrls => {  // this will contain the picture's url
                              imageUrl=signedUrls[0]
        })

    if (req.body.imageType === 'admin') {
      await connection.query("update user_profile set ? where user_id = ?", [{admin_profile_picture:imageUrl,last_updated_on:new Date()},req.user_id],
            (err, result, fields) => {
                if (err) {
                    res.status(500).send({ 
                        status: false,
        ...
        await connection.query("update student_profile set ? where student_id = ?", [{student_profile_picture:imageUrl,last_updated_on:new Date()},req.user_id],
            (err, result, fields) => {
                if (err) {
                    res.status(500).send({ 
                        status: false,
      ...
      await connection.query("update user_profile set ? where user_id = ?", [{school_cover_image:imageUrl,last_updated_on:new Date()},req.user_id,],
            (err, result, fields) => {
                if (err) {
                    res.status(500).send({ 
                        status: false,
...
router.post("/saveVideo", auth,async (req, res) => {
  ...
  try {
      ...
      let path = dir + "/";
      let videoObj = {
        'fileName': 'video'+'_'+req.user_id+'_'+req.body.classId+'_'+req.body.topicId+'_'+uuid(),
        'type': 'mp4'
      };
    ...
    fs.writeFile(`${path}${videoObj.fileName}`+'.'+`${videoObj.type}`, req.body.media, 'base64', function(err) {
      if (err) {
       res.status(400).send({
          status: false,
          message: err.message
    ...
    let videoUrl;
    await bucket.file(videoObj.fileName+'.mp4').getSignedUrl({
                                action: 'read',
                                expires: '03-09-2491'
                              }).then(signedUrls => {  // this will contain the picture's url
    ...
    let videoDetails = {
      ...
      await connection.query("insert into videos set ?",videoDetails,
            (err, result, fields) => {
                if (err) {
...
router.get("/getVideos", auth, async (req, res) => {
    try {
        await connection.query("SELECT video_id,class_id,topic_id,video_url FROM videos WHERE school_id=?", req.user_id,
            (err, results, fields) => {
              if (err) {
                    res.status(500).send({
                        status: false,
...
router.delete("/removeVideo/:id",auth, async (req, res) => {
    try {
        const _id = req.params.id;
        await connection.query("SELECT video_url FROM videos WHERE video_id = ?", _id,
            async (err, results, fields) => {
                if (err) {
                    res.status(500).send({
                        status: false,
              ...
              // Delete the file
                fileRef.delete().then(()=>{
                    connection.query("DELETE FROM videos WHERE video_id = ?", _id,
                      async (err, results, fields) => {
                          if (err) {
                              res.status(500).send({
                                  status: false,
